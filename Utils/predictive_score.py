...
def predictive_score(ori_data, generated_data):
    ...
    for itt in tqdm(range(iterations), desc='training predictor', total=iterations):
        ...
        optimizer.zero_grad()
        outputs = model(X_mb, torch.tensor(T_mb).cpu())  ##Recall this is changed from .to(device)
    ...
    # Prediction
    with torch.no_grad():
        pred_Y_curr = model(X_mb, torch.tensor(T_mb).cpu()) ##CHANGED AS ABOVE FROM .to(device)
