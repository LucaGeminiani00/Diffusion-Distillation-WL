...
class PredictiveScoreModel:
    def __init__(
        ...
        self.generated_time, self.generated_max_seq_len = extract_time(
            self.generated_data
        )
        ...
        self.model = PredictorModel(
            input_dim=self.dim - 1, hidden_dim=self.hidden_dim
        ).to(self.device)
    ...
    def train(self):
        ...
        for itt in tqdm(
            range(self.iterations), desc="Training Predictor", total=self.iterations
        ):
            ...
            X_mb = [
                torch.tensor(
                    self.generated_data[i][:-1, : (self.dim - 1)], dtype=torch.float32
                ).to(self.device)
                for i in train_idx
            ]
            ...
            Y_mb = [
                torch.tensor(
                    np.reshape(
                        self.generated_data[i][1:, (self.dim - 1)],
                        [len(self.generated_data[i][1:, (self.dim - 1)]), 1],
                    ),
                    dtype=torch.float32,
                ).to(self.device)
                for i in train_idx
            ...
            X_mb = torch.nn.utils.rnn.pad_sequence(
                X_mb, batch_first=True, padding_value=0
            )
            Y_mb = torch.nn.utils.rnn.pad_sequence(
                Y_mb, batch_first=True, padding_value=0
            )
    ...
    def compute_pred(self, fake_data):
        ...
        X_mb = [
            torch.tensor(
                self.ori_data[i][:-1, : (self.dim - 1)], dtype=torch.float32
            ).to(self.device)
            for i in train_idx
        ]
        Y_mb = [
            torch.tensor(
                np.reshape(
                    self.ori_data[i][1:, (self.dim - 1)],
                    [len(self.ori_data[i][1:, (self.dim - 1)]), 1],
                ),
                dtype=torch.float32,
            ).to(self.device)
            for i in train_idx
        ...
        fake_data = [
            torch.tensor(fake_data[i][:-1, : (self.dim - 1)], dtype=torch.float32).to(
                self.device
            )
            for i in range(len(fake_data))
        ]
        fake_t = [self.generated_time[i] - 1 for i in range(len(fake_data))]
        fake_data = torch.nn.utils.rnn.pad_sequence(
            fake_data, batch_first=True, padding_value=0
        )
