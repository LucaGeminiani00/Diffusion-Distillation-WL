...
def discriminative_score(ori_data, generated_data):
    ...
    for itt in tqdm(range(iterations), desc='training', total=iterations):
        ...
        optimizer.zero_grad()
        y_logit_real = model(X_mb, T_mb)
        y_logit_fake = model(X_hat_mb, T_hat_mb)
    ...
    with torch.no_grad():
        y_pred_real_curr = torch.sigmoid(model(test_x, test_t))
        y_pred_fake_curr = torch.sigmoid(model(test_x_hat, test_t_hat))
    ...
    fake_acc = accuracy_score(np.zeros(len(y_pred_fake_curr_bin)), y_pred_fake_curr_bin)
    real_acc = accuracy_score(np.ones(len(y_pred_real_curr_bin)), y_pred_real_curr_bin)
