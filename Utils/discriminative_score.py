...
class DiscriminativeScoreModel:
    def __init__(
        ...
        (
            ...
            self.ori_data, self.generated_data, self.ori_time, self.generated_time
        )

        self.model = DiscriminatorModel(
            input_dim=self.dim, hidden_dim=self.hidden_dim
        ).to(self.device)
    ...
    def train(self):
        for itt in tqdm(
            range(self.iterations), desc="Training Discriminator", total=self.iterations
        ):
            X_mb, T_mb = batch_generator(self.train_x, self.train_t, self.batch_size)
            X_hat_mb, T_hat_mb = batch_generator(
                self.train_x_hat, self.train_t_hat, self.batch_size
            )

            X_mb = torch.stack(
                [torch.tensor(x, dtype=torch.float32).to(self.device) for x in X_mb]
            )
            T_mb = torch.tensor(T_mb, dtype=torch.long).to(self.device)
            X_hat_mb = torch.stack(
                [torch.tensor(x, dtype=torch.float32).to(self.device) for x in X_hat_mb]
            )
    ...
    def compute_dis(self, fake_data):
        ...
        test_x = torch.stack(
            [torch.tensor(x, dtype=torch.float32).to(self.device) for x in self.test_x]
        )
        test_t = torch.tensor(self.test_t, dtype=torch.long).to(self.device)

        fake_data = torch.stack(
            [torch.tensor(x, dtype=torch.float32).to(self.device) for x in fake_data]
        )
        ...
        y_label_final = torch.cat(
            (
                torch.ones(y_pred_real_curr_bin.size(0)),
                torch.zeros(y_pred_fake_curr_bin.size(0)),
            ),
            dim=0,
        )
