...
def build_from_teacher(config, device, target = None): 
    if target is None:
        config['model']['target'] = 'Models.knowledge_distillation.progressive_distillation.Diffusion_FS'
        config['model']['params']['timesteps'] //= 2
        config['model']['params']['sampling_timesteps'] //= 2
        model = instantiate_from_config(config['model']).to(device)
        model.use_distillation = True
    else: 
