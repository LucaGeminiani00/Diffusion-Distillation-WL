...
class MLP(nn.Module):  # Adjusted MLP Model
    def __init__(self, input_dim, output_dim, resid_pdrop=0.0):
...
class Periodic(nn.Module):
    ...
    def forward(self, x):
        ...
        yh_hat = [yh[j] * g[j] for j in range(self.J)]  # (B, C, T / 2^(j+1))
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, timestep, mask=None, label_emb=None):
        a, att = self.attn(
            self.ln1(x, timestep, label_emb), mask=mask
        )  # Full attention #a = ([64, 24, 1024]) #att = [64, 24, 24]
        x = x + a
